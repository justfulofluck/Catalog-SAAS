...
const PublishView: React.FC = () => {
  ...
  const handlePublish = (catalog: any) => {
    setPublishingId(catalog.id);
    
    // Simulate API delay for publishing process
    setTimeout(() => {
        publishCatalog(catalog.id);
        setPublishingId(null);
        setShowShareModal(catalog.id);
    }, 1500);
  };
  ...
  const ShareModal = ({ catalogId, onClose }: { catalogId: string, onClose: () => void }) => {
    ...
    const publicUrl = `https://catalog.studio/view/${catalogId}`;

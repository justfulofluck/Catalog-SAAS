...
import { ArrowLeft, Save, Hash, AlignLeft, Image as ImageIcon, DollarSign, FolderTree, ChevronDown, Package, Info, Upload, Sparkles, RefreshCcw, Layers, CreditCard } from 'lucide-react';
...
const CreateProductForm: React.FC = () => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>, fieldKey: string = 'image') => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        handleInputChange(fieldKey, reader.result);
      };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Map dynamic fields back to standard Product structure + Custom Fields
    // If using a template, we assume specific mappings based on 'id' conventions in the schema:
    // prod_name -> name
    // price -> price
    // main_image -> image
    // description -> description
    // category -> categoryId (if mapped)
    // ALL others -> customFields

    let finalProduct: any = {
      id: `p-${Date.now()}`,
      currency: formData.currency || defaultCurrency,
      sku: formData.sku, // SKU is handled separately in the UI typically, but if template has 'sku' field, use it.
    };
    ...
    if (activeTemplate) {
      ...
      // Fallbacks if template didn't cover core fields
      if (!finalProduct.name) finalProduct.name = formData.name || 'Untitled';
      if (!finalProduct.price) finalProduct.price = parseFloat(formData.price) || 0;
      if (!finalProduct.image) finalProduct.image = formData.image || 'https://picsum.photos/seed/default/400/400';
      if (!finalProduct.description) finalProduct.description = formData.description;
      if (!finalProduct.categoryId) finalProduct.categoryId = formData.categoryId;

    } else {
      // Standard legacy behavior
      finalProduct.name = formData.name;
      finalProduct.price = parseFloat(formData.price) || 0;
      finalProduct.description = formData.description;
      finalProduct.image = formData.image || 'https://picsum.photos/seed/default/400/400';
      finalProduct.categoryId = formData.categoryId;
    }
  ...
  const renderDynamicField = (field: FormField) => {
    ...
    switch (field.type) {
      case 'textarea':
        return (
          <textarea
            className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 text-base font-medium text-slate-700 dark:text-slate-300 min-h-[120px] focus:border-indigo-600 dark:focus:border-indigo-400 outline-none"
      ...
      case 'boolean':
        return (
          <div className="flex gap-2">
            <button type="button" onClick={() => handleInputChange(field.id, true)} className={`flex-1 py-3 rounded-xl text-xs font-black uppercase tracking-widest border transition-all ${value === true ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>Yes</button>
            <button type="button" onClick={() => handleInputChange(field.id, false)} className={`flex-1 py-3 rounded-xl text-xs font-black uppercase tracking-widest border transition-all ${value === false ? 'bg-slate-800 text-white border-slate-800' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>No</button>
          </div>
        );
      case 'image':
        return (
          <div className="space-y-3">
            {value ? (
              ...
              <label className="w-full h-32 flex flex-col items-center justify-center border-2 border-dashed border-slate-300 rounded-2xl cursor-pointer hover:border-indigo-500 hover:bg-indigo-50 transition-colors">
                <Upload size={24} className="text-slate-400 mb-2" />
                <span className="text-xs font-bold text-slate-500">Upload {field.label}</span>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[32px] shadow-sm border border-slate-100 dark:border-slate-800 p-8 space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <FolderTree className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-600" size={18} />
                    <select
                      value={formData.categoryId}
                      onChange={(e) => handleInputChange('categoryId', e.target.value)}
                      className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl pl-12 pr-10 py-4 text-base font-bold text-slate-800 dark:text-white focus:border-indigo-600 dark:focus:border-indigo-400 outline-none appearance-none"
                    >
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-[32px] shadow-sm border border-slate-100 dark:border-slate-800 p-8 space-y-8">
                ...
                <div className="space-y-6">
                  ...
                  {/* Always include SKU generation for tracking */}

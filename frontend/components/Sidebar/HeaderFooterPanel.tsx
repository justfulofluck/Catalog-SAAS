...
const HeaderFooterPanel: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#161616] text-white font-sans overflow-hidden select-none">
      {/* Panel Top Header */}
      <div className="h-14 px-4 border-b border-[#262626] flex items-center justify-between shrink-0 bg-[#161616]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="p-3.5 rounded-[4px] border border-[#262626] bg-[#1a1a1a] transition-all">
          <label className="flex items-center justify-between cursor-pointer">
            <span className="text-xs font-bold text-white">Enable Header</span>
            <input
              type="checkbox"
          ...
          {catalog.hasHeader && (
            <div className="mt-3 pt-3 border-t border-[#262626] space-y-3 animate-in fade-in duration-200">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-[#888888] uppercase tracking-wider">Height from top</span>
                <div className="flex items-center gap-1 border border-[#333] rounded-[4px] px-2 py-0.5 w-16 bg-[#121212]">
                  <input
                    type="number"
                    min="3"
                    max="30"
                    ...
                    className="w-full bg-transparent outline-none text-[11px] font-bold text-[#E2DCC8] text-right [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                  />
              ...
              <input
                ...
                className="w-full h-1.5 bg-[#262626] rounded-[4px] appearance-none cursor-pointer accent-[#0F3D3E]"
              />
              ...
              <div className="flex gap-2 pt-1">
                {catalog.headerElements?.some(el => el.type === 'text') ? (
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-[#0F3D3E]/15 border border-[#0F3D3E]/40 rounded-[6px] text-[#E2DCC8] text-[10px] font-bold hover:bg-[#0F3D3E]/25 transition-colors"
                  >
                    <Type size={13} /> Select Text
                  </button>
                ) : (
                  <button
                    onClick={() => addHeaderElement({ id: `header-text-${Date.now()}`, type: 'text', x: (catalog.marginLeft || 0) + 10, y: catalog.marginTop || 0, width: 200, height: 30, text: 'New Text', fontSize: 14, fontFamily: 'Inter', fill: '#cccccc', fontWeight: 'bold', rotation: 0, opacity: 1, zIndex: 10, verticalAlign: 'middle' })}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 border border-[#333] rounded-[6px] text-[10px] font-bold text-white bg-[#222222] hover:bg-[#2a2a2a] transition-colors"
                  >
        ...
        <div className="p-3.5 rounded-[4px] border border-[#262626] bg-[#1a1a1a] transition-all">
          <label className="flex items-center justify-between cursor-pointer">
            <span className="text-xs font-bold text-white">Enable Footer</span>
            <input
              type="checkbox"
          ...
          {catalog.hasFooter && (
            <div className="mt-3 pt-3 border-t border-[#262626] space-y-3 animate-in fade-in duration-200">
              ...
              <label className="flex items-center justify-between cursor-pointer pt-2 border-t border-[#262626]">
                <span className="text-[11px] font-bold text-white">Show Page Numbers</span>
                <input
                  type="checkbox"

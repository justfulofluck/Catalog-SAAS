...
const LayersPanel: React.FC = () => {
  const { catalog, currentPageIndex, selectedElementIds, setSelectedElementIds, setHoveredElementId, updateElement, setElementOrder, products } = useStore();
  ...
  return (
    <div className="flex flex-col h-full bg-white border-r w-[320px] shrink-0 z-10 shadow-[20px_0_60px_rgba(0,0,0,0.05)] animate-in slide-in-from-left-4 duration-500 font-sans">
      ...
      <div 
        ref={listRef}
        className="flex-1 overflow-y-auto p-4 space-y-2 bg-slate-50/30 custom-scrollbar"
      >
        {elements.length === 0 ? (
          ...
          elements.map((el) => (
            <div 
              ...
              className={`group flex items-center gap-3 p-3 rounded-2xl border transition-all cursor-pointer ${selectedElementIds.includes(el.id) ? 'bg-indigo-600 border-indigo-600 shadow-lg shadow-indigo-600/20' : 'bg-white border-slate-100 hover:border-indigo-200 shadow-sm'}`}
            >
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-xs font-black truncate transition-colors ${selectedElementIds.includes(el.id) ? 'text-white' : 'text-slate-800'}`}>
                  {getLabel(el)}
                </p>
                <div className="flex items-center gap-2 mt-1">
                   <span className={`text-[8px] font-black uppercase tracking-widest opacity-60 ${selectedElementIds.includes(el.id) ? 'text-white' : 'text-slate-400'}`}>{el.type === 'product-block' ? 'Unified Card' : el.type}</span>
